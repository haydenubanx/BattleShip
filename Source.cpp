...
//Function prototypes
void updateShipDetails(char shMap[][NUM_COLUMNS], bool posChecked[][NUM_COLUMNS], bool shipTrack1[][NUM_COLUMNS],
                       bool shipTrack2[][NUM_COLUMNS],
                       bool shipTrack3[][NUM_COLUMNS], bool shipTrack4[][NUM_COLUMNS], bool shipTrack5[][NUM_COLUMNS],
                       bool shipTrack6[][NUM_COLUMNS],
                       bool shipTrack7[][NUM_COLUMNS]);

void fire(int xCoordinate, int yCoordinate, char sMap[][NUM_COLUMNS], char sMapBlank[][NUM_COLUMNS],
          bool shipTra1[][NUM_COLUMNS],
          bool shipTra2[][NUM_COLUMNS], bool shipTra3[][NUM_COLUMNS], bool shipTra4[][NUM_COLUMNS],
          bool shipTra5[][NUM_COLUMNS],
...
int main() {
    ...
    cout << "The program will let you know if you hit, missed, or hit again as well as when all of the ships are sunk."
         << endl;
    cout << "To see a current map of your guesses enter 0 0 for your x and y coordinates." << endl << endl;
    cout << "Enter -1 for either value to quit." << endl << endl;
    ...
    //Enable exceptions for file input
...
//Open file and loop through each row and column to import grid
    ...
    updateShipDetails(shipMap, positionAlreadyChecked, shipTracker1, shipTracker2, shipTracker3, shipTracker4,
                      shipTracker5,
    ...
    while (allSunkCheck != true) {
        ...
        if (xCord == 0 && yCord != 0 || xCord != 0 && yCord == 0) {
            cout << "Enter 0 for both values to see map. Valid indexes for shots are 1-25" << endl;
            continue;
        }

        if (xCord < 0 || xCord > 25 || yCord < 0 || yCord > 25) {
            cout << "Coordinates out of bounds! Please enter values between 1 and 25." << endl;
            continue;
        }
        ...
        for (int j = 0; j < 25; j++) {
            ...
            if (shMap[i][j] == '#' && posChecked[i][j] == false) {
                ...
                if (ship1.length == 0) {
                    ...
                    for (int p = 1; p <= 5; p++) {
                        //Check spaces directly below this location up to 5 spaces below, with boundary check
                        if ((i + p < 25) && (shMap[i + p][j] == '#')) {
                            ship1.length++;
                            shipTrack1[i + p][j] = true;
                            posChecked[i + p][j] = true;
                        }

                        //Check spaces to the right of this position up to 5 spaces away, with boundary check
                        if ((j + p < 25) && (shMap[i][j + p] == '#')) {
                            ship1.length++;
